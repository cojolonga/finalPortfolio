...
async function getEditProjects(): Promise<Project[]> {
  try {
    const manifestPath = join(process.cwd(), 'app', '_data', 'manifest.json')
    const manifestContent = readFileSync(manifestPath, 'utf-8')
    const manifest: Manifest = JSON.parse(manifestContent)
    return manifest.projects.filter(p => p.category === 'edits')
  } catch (error) {
    return []
  }
}
...
export default async function EditsPage() {
  const projects = await getEditProjects()
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-20">
            ...
            <nav className="hidden md:flex space-x-8">
              ...
              <Link href="/edits" className="text-criforge-accent font-bold font-cinzel text-lg tracking-wide">
                EDITS
              </Link>
      ...
      <section className="py-20 px-4 bg-gradient-to-b from-inferno-bg to-inferno-gray">
        <div className="max-w-7xl mx-auto text-center">
          ...
          <p className="text-xl text-inferno-fg/80 max-w-3xl mx-auto">
            Creative photo manipulation and composite work that transforms ordinary images into extraordinary art
          </p>
        </div>
      </section>
        ...
        <div className="max-w-7xl mx-auto">
          {projects.length === 0 ? (
            <div className="text-center text-inferno-fg/60">
              <p className="text-xl mb-4">No edit projects found</p>
              <p>Add content to the /content/edits folder</p>
            </div>
          ) : (
